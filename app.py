...
# ── FIX 1: mentor.html -> mentor-dashboard.html redirect ──
@app.route('/mentor.html')
def mentor_redirect():
    return redirect('/mentor-dashboard.html')
...
def serve_static_files(filename):
    ...
    return f"""<!DOCTYPE html><html><head><title>ConnectU - Sahifa topilmadi</title></head>
<body style="background:#040810;color:#EEF2FF;font-family:sans-serif;padding:40px;">
<h1 style="color:#F0A500;">404 - Sahifa topilmadi</h1>
<p>Fayl: <code>{filename}</code></p><ul>{links}</ul>
</body></html>""", 404
...
def confirm_role():
    ...
    db.session.commit()
    flask_session['role'] = user.role
    # ── FIX: mentor.html emas mentor-dashboard.html ──
...
def get_me():
    ...
    # ── FIX: Mentor profil to'liq ma'lumot ──
    if user.role == 'mentor' and user.mentor_profile:
        mp = user.mentor_profile
        # Individual va guruh narxlarini olish (model da bo'lsa)
        ind_price = getattr(mp, 'individual_price', None) or 8000
        grp_price = getattr(mp, 'group_price', None) or 2000
        ...
        user_dict['mentor_profile'] = {
            ...
            'subjects': getattr(mp, 'subjects', []) or [],
            'languages': getattr(mp, 'languages', []) or [],
        }
...
# ==========================================
# UNIVERSITIES & FACULTIES — FIX: har bir
# universitetning o'z fakultetlari
# ==========================================
...
def handle_universities():
    if request.method == 'GET':
        ...
        for u in unis:
            u_dict = u.to_dict()
            # ── FIX: Har bir universitetning o'z fakultetlari ──
            # university_id bo'yicha filter qilamiz
            ...
            u_dict['faculties'] = fac_list  # Har uni o'z fakultetlari
...
def handle_university(uni_id):
    ...
    user = get_current_user()
    if request.method == 'GET':
        # ── FIX: university_id bo'yicha to'g'ri filter ──
...
def handle_faculties(uni_id):
    ...
    user = get_current_user()
    if request.method == 'GET':
        # ── FIX: Faqat shu universitetning fakultetlari ──
    ...
    data = request.json
    faculty = Faculty(
        university_id=uni_id,  # ── FIX: to'g'ri uni_id ──
...
def get_mentors():
    ...
    for m in mentors:
        ...
        if user:
            ...
            m_dict['individual_price'] = getattr(m, 'individual_price', None) or 8000
            m_dict['group_price'] = getattr(m, 'group_price', None) or 2000
...
def get_mentor_detail(mentor_id):
    ...
    # Videolar (abuturyent ko'rishi uchun — faqat bepul)
    try:
        videos = Material.query.filter(
            Material.material_type.in_(['video', 'youtube'])
        )
        if hasattr(Material, 'mentor_id'):
            videos = videos.filter(Material.mentor_id == mentor.id)
        videos = videos.order_by(Material.created_at.desc()).limit(10).all()
        mentor_dict['videos'] = []
        for v in videos:
            v_dict = v.to_dict()
        ...
        for field in ['university', 'faculty', 'year', 'bio', 'gpa']:
            if field in data:
                ...
                if val is not None and str(val).strip() != '':
                    ...
                    setattr(mp, field, val)
        # Individual va guruh narxlari
        for field in ['individual_price', 'group_price']:
            if field in data and hasattr(mp, field):
...
def get_sessions():
    ...
    for s in sessions:
        ...
        # ── FIX: session price mentor profilidan ──
        ind_price = getattr(mentor_profile, 'individual_price', None) or 8000 if mentor_profile else 8000
        grp_price = getattr(mentor_profile, 'group_price', None) or 2000 if mentor_profile else 2000
        ...
        result.append({
            ...
            'notes': s.notes or '',
            'points_awarded': s.points_awarded or price,
            'student_rating': s.student_rating,
            'student_review': s.student_review,
...
def create_session():
    ...
    # ── FIX: Mentorga bildirishnoma yuborish ──
    sess = Session(
    ...
    # Mentorga xabar
    mentor_user = db.session.get(User, mentor.user_id)
    if mentor_user:
        db.session.add(Notification(
            user_id=mentor_user.id,
            title='📅 Yangi sessiya so\'rovi',
            body=f'{user.full_name} {session_type} sessiya so\'radi.',
            type='session'
        ))
        if mentor_user.telegram_id:
            send_telegram_message(
                mentor_user.telegram_id,
                f"📅 <b>Yangi sessiya so'rovi!</b>\n\n"
                f"👤 Talaba: {user.full_name}\n"
                f"📚 Tur: {'Individual' if session_type == 'individual' else 'Guruh'}\n"
                f"ConnectU panelida tasdiqlang."
            )
...
@app.route('/api/sessions/<string:session_id>/complete', methods=['POST'])
def complete_session(session_id):
    """Sessiyani tugatish va mentorga ball berish"""
    user = get_current_user()
    if not user or user.role != 'mentor':
        return jsonify({'success': False, 'error': "Ruxsat yo'q"}), 403
    s = db.session.get(Session, session_id)
    ...
    s.status = 'completed'
    s.points_awarded = price

    # Balance yangilash
    mentor.balance = (mentor.balance or 0) + price
    ...
    db.session.commit()
    return jsonify({'success': True, 'points_awarded': price})

@app.route('/api/sessions/<string:session_id>/rate', methods=['POST'])
def rate_session(session_id):
    """Talaba tomonidan sessiyaga baho berish"""
    user = get_current_user()
    if not user:
        return jsonify({'success': False, 'error': 'Kirilmagan'}), 401
    s = db.session.get(Session, session_id)
    ...
    if s.status != 'completed':
        return jsonify({'success': False, 'error': "Sessiya tugallanmagan"}), 400
    data = request.json or {}
    rating = data.get('rating')
    review = data.get('review', '')
    if not rating or not (1 <= int(rating) <= 5):
        return jsonify({'success': False, 'error': "Baho 1-5 oralig'ida bo'lishi kerak"}), 400
    s.student_rating = int(rating)
    s.student_review = review
    
    # Mentor reytingini yangilash
    mentor = db.session.get(MentorProfile, s.mentor_id)
    if mentor:
        rated_sessions = Session.query.filter(
            Session.mentor_id == mentor.id,
            Session.student_rating.isnot(None)
        ).all()
        if rated_sessions:
            mentor.rating = sum(x.student_rating for x in rated_sessions) / len(rated_sessions)
            mentor.total_reviews = len(rated_sessions)
...
def mentor_student_id_status():
    ...
    return jsonify({
        ...
        'status': 'verified' if mentor.is_verified else ('pending' if mentor.student_id_url else 'none'),
        'days_left': days_left,
        'created_at': mentor.created_at.isoformat() if mentor.created_at else None,
        'verified_at': mentor.verified_at.isoformat() if getattr(mentor, 'verified_at', None) else None
    })
...
def add_mentor_certificate():
    """
    Sertifikat qo'shish: JSON (faqat ma'lumotlar) yoki
    multipart/form-data (fayl bilan) yuklash mumkin
    """
    user = get_current_user()
    if not user or user.role != 'mentor':
        return jsonify({'success': False, 'error': "Ruxsat yo'q"}), 403
    ...
    # Multipart (fayl bilan) yoki JSON?
    if request.content_type and 'multipart' in request.content_type:
        ...
        if 'file' in request.files:
            ...
            if f and f.filename:
                ...
                filename = secure_filename(f"cert_{mentor.id}_{uuid.uuid4().hex[:8]}.{ext}")
                save_path = os.path.join('uploads', 'certificates', filename)
    ...
    if date_str:
        try:
            issued_date = date.fromisoformat(date_str)
        except:
            pass

    cert = MentorCertificate(
        mentor_id=mentor.id, title=title,
        issuer=issuer or None, issued_date=issued_date,
        file_url=file_url
    )
    db.session.add(cert)
    db.session.commit()
...
def mentor_withdraw():
    ...
    for adm in admins:
        if adm.telegram_id:
            ...
            send_telegram_message(adm.telegram_id, msg)
...
@app.route('/api/mentor/withdrawals', methods=['GET'])
def mentor_withdrawal_history():
    """Mentor o'zining pul yechish tarixini ko'rish"""
    user = get_current_user()
    if not user or user.role != 'mentor':
        return jsonify({'success': False, 'error': "Ruxsat yo'q"}), 403
    mentor = user.mentor_profile
    ...
    withdrawals = Withdrawal.query.filter_by(mentor_id=mentor.id)\
...
def mentor_dashboard():
    ...
    def session_to_dict(s):
        ...
        return {
            ...
            'meet_link': s.meet_link, 'points_awarded': s.points_awarded or price,
            'duration_min': s.duration_min, 'price': price,
        }
    ...
    total_views = 0
    try:
        vq = Material.query.filter(Material.material_type.in_(['video', 'youtube']))
        if hasattr(Material, 'mentor_id'):
...
# ==========================================
# MENTOR VIDEOS — FIX: mentor_id saqlash
# ==========================================
...
def get_mentor_videos():
    ...
    try:
        ...
        videos_q = videos_q.order_by(Material.created_at.desc())
        videos = videos_q.all()
    except Exception:
        videos = []
...
def add_mentor_video():
    ...
    if not thumbnail:
        ym = re.search(r'(?:youtube\.com/watch\?v=|youtu\.be/)([^&\s]+)', url)
        if ym:
            thumbnail = f"https://img.youtube.com/vi/{ym.group(1)}/hqdefault.jpg"
    
    video = Material(title=title, material_type=mat_type, url=url)
    optional = {
        'description': data.get('description', ''),
        'access_type': data.get('access_type', data.get('access', 'free')),
        'mentor_id': mentor.id,  # ── FIX: mentor_id saqlanadi ──
        'views': 0,
        'likes': 0,
        'thumbnail': thumbnail,
    }
    ...
    result['access_type'] = getattr(video, 'access_type', 'free') or 'free'
    result['access'] = result['access_type']
    result['thumbnail'] = thumbnail
    result['likes'] = 0
    result['views'] = 0
    return jsonify({'success': True, 'video': result, 'material': result})
...
def increment_video_view(video_id):
    ...
    if hasattr(video, 'views'):
        ...
        db.session.commit()
    return jsonify({'success': True, 'views': getattr(video, 'views', 0)})

@app.route('/api/mentor/videos/<string:video_id>/like', methods=['POST'])
def toggle_video_like(video_id):
    """Abuturyent video like bosishi"""
    user = get_current_user()
    if not user:
        return jsonify({'success': False, 'error': 'Kirilmagan'}), 401
    video = db.session.get(Material, video_id)
    ...
    if hasattr(video, 'likes'):
        ...
        db.session.commit()
    return jsonify({'success': True, 'likes': getattr(video, 'likes', 0)})
...
def add_material_legacy():
    """Eski endpoint — mentor-videos.html ishlatishi uchun"""
    user = get_current_user()
    if not user:
        return jsonify({'success': False, 'error': 'Kirilmagan'}), 401
    data = request.get_json(force=True, silent=True) or {}
    ...
    # Agar mentor bo'lsa, mentor_id ham saqlaymiz
    if user.role == 'mentor' and user.mentor_profile:
        optional['mentor_id'] = user.mentor_profile.id
    ...
    result['likes'] = 0
    result['views'] = 0
    return jsonify({'success': True, 'material': result, 'video': result})
...
def admin_stats():
    ...
    if not admin_user or admin_user.role not in ['admin', 'superadmin']:
        return jsonify({'success': False, 'error': "Ruxsat yo'q"}), 403
    return jsonify({'success': True, 'stats': {
        'students': User.query.filter_by(role='student').count(),
        'mentors': MentorProfile.query.filter_by(is_verified=True).count(),
...
@app.route('/api/admin/mentors/<string:mentor_id>', methods=['GET'])
def admin_get_mentor_detail(mentor_id):
    """Admin mentorning to'liq ma'lumotlarini ko'radi (pul yechish oldidan)"""
    admin_user = get_current_user()
    if not admin_user or admin_user.role not in ['admin', 'superadmin']:
        return jsonify({'success': False, 'error': "Ruxsat yo'q"}), 403
    ...
    if not mentor:
        abort(404)
    u = db.session.get(User, mentor.user_id)
    
    # Sessiyalar
    sessions = Session.query.filter_by(mentor_id=mentor.id)\
        .order_by(Session.created_at.desc()).limit(20).all()
    sessions_data = []
    for s in sessions:
        student = db.session.get(User, s.student_id)
        sessions_data.append({
            'id': s.id, 'status': s.status, 'session_type': s.session_type,
            'student_name': student.full_name if student else "Noma'lum",
            'scheduled_at': s.scheduled_at.isoformat() if s.scheduled_at else None,
...
def admin_verify_mentor(mentor_id):
    ...
    if hasattr(mentor, 'verified_at'):
        mentor.verified_at = datetime.utcnow()
    if hasattr(mentor, 'verified_by'):
        mentor.verified_by = admin_user.id
...
def admin_get_mentor_documents(mentor_id):
    ...
    return jsonify({'success': True, 'student_id_url': mentor.student_id_url,
                    'is_verified': mentor.is_verified,
                    'verified_at': getattr(mentor, 'verified_at', None)})
    ...
    if user:
        ...
        if user.telegram_id:
            send_telegram_message(
                ...
                f"💰 Summa: <b>{wd.amount:,} so'm</b>\n"
                f"💳 Karta: •••• {wd.card_last4}\n"
                f"⏳ Tez orada kartangizga o'tkaziladi."
            )
    return jsonify({'success': True})

@app.route('/api/admin/withdrawals/<string:wd_id>/upload-check', methods=['POST'])
def admin_upload_check(wd_id):
    """
    Admin chek yuki — fayl yoki URL.
    Tasdiqlangandan keyin chek yuklanadi va mentorga xabar yuboriladi.
    """
    admin_user = get_current_user()
    if not admin_user or admin_user.role not in ['admin', 'superadmin']:
        return jsonify({'success': False, 'error': "Ruxsat yo'q"}), 403
    ...
    if 'check' in request.files:
        f = request.files['check']
        if f and f.filename:
            ext = f.filename.rsplit('.', 1)[-1].lower() if '.' in f.filename else 'pdf'
            filename = secure_filename(f"check_{wd_id[:8]}_{uuid.uuid4().hex[:6]}.{ext}")
            save_path = os.path.join('uploads', 'checks', filename)
            os.makedirs(os.path.dirname(save_path), exist_ok=True)
            f.save(save_path)
            check_url = f"/uploads/checks/{filename}"
    elif request.is_json:
        data = request.json or {}
        check_url = data.get('check_url', '').strip()
    ...
    if user:
        db.session.add(Notification(
            user_id=user.id,
            title="✅ Pul yechildi!",
            body=f"{wd.amount:,} so'm kartangizga o'tkazildi. Chekni ko'ring.",
            type='points'
        ))
        db.session.commit()
        if user.telegram_id:
            send_telegram_message(
                user.telegram_id,
                f"✅ <b>Pul yechildi!</b>\n\n"
                f"💰 <b>{wd.amount:,} so'm</b> kartangizga o'tkazildi.\n"
                f"💳 Karta: •••• {wd.card_last4}\n"
                f"📄 Chekni ConnectU panelida ko'rishingiz mumkin."
            )
    
    return jsonify({'success': True, 'check_url': check_url})
...
def admin_reject_withdrawal(wd_id):
    ...
    if user and user.telegram_id:
        send_telegram_message(
            ...
            f"💰 Summa: {wd.amount:,} so'm\n"
            f"📝 Sabab: {wd.admin_note or 'Ko\'rsatilmagan'}"
        )
    return jsonify({'success': True})
...
def cleanup_unverified_mentors():
    ...
    unverified = MentorProfile.query.filter(
        ...
        MentorProfile.student_id_url.is_(None),
